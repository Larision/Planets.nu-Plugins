...
function wrapper1() { // wrapper for injection
	...
	var roboMaxPlugin = {
		...
		displayRoboMax: function (view) {
			...
			//check homesector
			plg.homeSector = vgap.isHomeSector();
			...
			if (view == 0) {
				...
				$('#setColonistTaxesCheck').click(function () {
					const climateTaxes = document.getElementById('climateTaxes');
					console.log("setColonistTaxesCheck CLICKED");
					if (plg.setColonistTaxes == true) {
						plg.setColonistTaxes = false;
						climateTaxes.style.display = 'none'; //oculta anidada
					} else {
				...
				// Cambia settings al clickar
				$('#homesectorCheck').click(function () {
					const homeSectorOptions = document.getElementById('homesectorOptions');
					console.log("Home sector CLICKED");
					if (vgap.game.gametype == 100) {
						if (plg.homeSector == true) {
							plg.homeSector = false;
							homeSectorOptions.style.display = 'none'; // Ocultar las opciones anidadas
						} else {
							plg.homeSector = true;
							homeSectorOptions.style.display = 'block';
						}
					} else {
						// Si homeSector es false, deshabilitar la casilla y asegurarse de que esté sin marcar
						$(this).prop("checked", false);
						plg.homeSector = false;
						homeSectorOptions.style.display = 'none'; // Ocultar las opciones anidadas
					}
					console.log("Home Sector is now: " + plg.homeSector);
				});
			...
			if (view == 2) {
				...
				html += "<h2>Options</h2><br/><h3 style='color:yellow'>Automatically unload megacredits</h3><p>If this option is checked, RoboMax will unload money to your planets from orbiting ships.</p><h3 style='color:yellow'>Automatically unload cargo</h3><p>If this option is checked, RoboMax will unload colonists, supplies, and minerals to your planets.</p><h3 style='color:yellow'>Build factories and mines</h3><p>RoboMax will automatically build factories and mines. The target number of factories is the maximum possible. The target number of mines is the amount needed to mine the planet out in 30 turns.</p><h4 style='color:cyan'>&nbsp;&nbsp;&nbsp;Decrease Mines and Factories to #Target. </h4><p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Robomax will check targetmines and targetfactories and if lower than built will tax colonist to 39% happy, riots will destroy mines and factories, when target reached will back to normal taxes.BE CAREFUL WITH THIS OPTION!</p><h3 style='color:yellow'>Automatically build defense posts where needed. </h3><p>RoboMax will scan for enemy activity and build planetary defense posts where it thinks they are needed. The default number of defense posts for a planet is 15, and it is 57 for a planet with a starbase. If enemy activity is detected within a 200 LY radius, it will attempt to max defense posts. Fewer defense posts will be built in the early turns of the game.</p><h3 style='color:yellow'>Tax colonists</h3><p>RoboMax will tax colonists as follows. If the population can't grow, use safe tax 40. However, if your race can build appropriate terraformers that can change the temperature so that clans can grow, use flat tax 70. If the population is less that 5000 clans and the population can grow, set the tax rate to zero. Otherwise, if the population is less than or equal to 6.6 million and the population can grow, use growth tax. If the planet is still building factories, use flat tax instead of growth tax to get money faster. Otherwise, if the population is greater than 6.6 million, use flat tax 70.</p><h4 style='color:cyan'>&nbsp;&nbsp;&nbsp;No tax with bad climate</h4><p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;This is intended for horwasp enemies and you can deal with rock pods and happiness drops.</p><h3 style='color:yellow'>Tax natives</h3><p>RoboMax will tax natives as follows. If the population can't grow, use safe tax 40. However, if your race can build appropriate terraformers that can change the temperature so that clans can grow, use flat tax 70. If the population is less that 2500 clans and the population can grow, set the tax rate to zero. Otherwise, if the population is less than or equal to 6.6 million and the population can grow, use growth tax. If the planet is still building factories, use flat tax instead of growth tax to get money faster. Otherwise, if the population is greater than 6.6 million, use flat tax 70 unless the natives are bovinoid (in that case use growth tax).</p><h3 style='color:yellow'>Manage planetary friendly codes</h3><p>If this option is checked, RoboMax will assign new friendly codes to all planets. The rules are as follows: 'att', 'nuk', 'bum', 'edf', 'dmp' are changed to a random permutation of the same code. E.g., 'Att' to 'atT'. 'pbX' and 'mfX' codes are altered in a similar way. E.g., 'pb1' to 'pB1'. Codes beginning with an 'x' are left unaltered. The rationale is that Superspy Deluxe cannot be used to set codes that start with 'x', so these codes are assumed to be special and intentional. These are the codes that you can give to your allies. All other codes are replaced with a random alphanumeric code. The algorithm makes it impossible for a special code to be generated this way.</p><h4 style='color:cyan'>&nbsp;&nbsp;&nbsp;Home sector special options.</h4><p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Options will appear if you in home sector.</p><h4 style='color:green'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Max Growth priority. </h4><p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Robomax wont tax colonists to ensure maximum growth.</p><h4 style='color:green'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Growth-Tax priority. </h4><p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Robomax basically will use growth tax for colonists.</p>";
				html += "</td></tr>"
				html += "</table><br /></div>";
