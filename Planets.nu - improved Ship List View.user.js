...
function wrapper() { // wrapper for injection
    ...
    vgapDashboard.prototype.showShips = function (view) {
        ...
        for (var i = 0; i < vgap.myships.length; i++) {
            ...
            if (view == 1) {
                var carga = ship.supplies + ship.duranium + ship.tritanium + ship.molybdenum + ship.ammo + ship.clans;
                var cargostyle = "";
                if (carga == hull.cargo)
            ...
            if ((view == 0) || (view == 6)) {
                ...
                // Icono de estado
                temphtml += "<td class='toggle-cell'><i style='" + readystyle + "' class='" + readyclass + "' id='Icon" + i + "' onclick='toggleReadyStatus(" + i + ");'></i></td>";
            }
                    ...
                    for (var count = 0; count < vgap.myships.length; count++) {
                        ...
                        if ((showDestination == 0) && (vgap.myships[count].x == enteredShip.x) && (vgap.myships[count].y == enteredShip.y)) {
                            ...
                            if (ship2.readystatus > 0)
                                ...
                                html += "<td><input type='checkbox' id='CheckA" + count + "' onchange='setCB(this);'/></td>";
    ...
    setFleetDest = function (ship__ship) {
        ...
        if (command == 1) fleetName = returnFleetName2(ship);
        var count = 0;
        var lowFuel = 0;
        if (command == 1) //in a labeled fleet
            for (var i = 0; i < vgap.myships.length; i++) {
                if (returnFleetName2(vgap.myships[i]) == fleetName) {
                    count++;
